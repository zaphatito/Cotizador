...
def read_sheet2_presentations(path_xlsx: str) -> pd.DataFrame:
    ...
    for c in df.columns:
        ...
        elif "genero" in cl or "genero" in cl or "género" in str(c).lower():
    ...
    for key in price_priority:
        nk = _norm_txt(key)
        # probar variantes sin espacios y con punto
        candidates = [nk, nk.replace(" ", ""), nk.replace(".", ""), nk.replace(" ", "").replace(".", "")]
        found = None
        for cand in candidates:
            # búsqueda por inclusión (p. ej. "precio maximo" dentro de "precio maximo s/ igv")
    ...
    return out.loc[~mask_all_empty].reset_index(drop=True)

def norm_pres_code(c: str) -> tuple[str, bool, bool]:
    """
    Devuelve (codigo_normalizado, requiere_botella, ignorar)
    - 'PZA' se ignora
    - 'E100' -> ('0100', requiere_botella=True)
    - 'C240' -> ('C240', requiere_botella=False)
    - 'XXX' numérico de 3-4 dígitos -> requiere_botella=True
    """
    if not c:
        return "", False, False
    cu = c.strip().upper()
